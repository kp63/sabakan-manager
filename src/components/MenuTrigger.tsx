...
import { MenuItem } from "@/components/ContextMenuTrigger";
...
const MenuTrigger = ({ items, children, className, openedClassName, ...props }: Props) => {
  return (
    <Menu as="div" className="relative text-left z-50 select-none">
      <Menu.Button as="div">
        {({ open }) => (
...
export const MenuItems = ({ items, backdrop, ...props }: { items: MenuItem[], static?: boolean, backdrop?: boolean }) => {
  return (
    ...
    >
      <Menu.Items static={props.static} className={'absolute right-0 mt-2 origin-top-right' +
